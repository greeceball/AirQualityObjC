...
#import "CHCityAirQualityController.h"
...
@implementation CHCityAirQualityController
...
+ (void)fetchDataForCity:(NSString *)city state:(NSString *)state country:(NSString *)country completion:(void (^)(CHCityAirQuality * _Nullable))completion
{
    ...
    NSURLComponents *urlComponents = [[NSURLComponents alloc] initWithURL:cityURL resolvingAgainstBaseURL:true];
        ...
        [[[NSURLSession sharedSession] dataTaskWithURL:finalURL completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
            ...
            if (data)
            {
                ...
                CHCityAirQuality *cityAQI = [[CHCityAirQuality alloc] initWithDictionary:dataDict];
                completion(cityAQI);
            }
        }] resume];
    }
